  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between border-b border-border/20 pb-2">
        ...
        {!isLoading && !error && safeData.length > 5 && (
          <div className="hidden md:flex gap-1.5">
            <button
              onClick={() => handleScroll('left')}
              aria-label={`Scroll ${title} left`}
              className="w-10 h-10 flex items-center justify-center rounded-lg bg-muted/40 hover:bg-muted border border-border/30 text-muted-foreground hover:text-white cursor-pointer transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary"
            >
      ...
      {isLoading ? (
        ...
        <div className="py-12 border border-dashed border-destructive/20 rounded-2xl flex flex-col justify-center items-center text-center p-6 bg-destructive/5">
          <FolderOpen className="size-8 text-destructive/40 mb-2 animate-pulse" />
        ...
        <div className="py-12 border border-dashed border-border/20 rounded-2xl flex flex-col justify-center items-center text-center p-6 bg-muted/5">
          <FolderOpen className="size-8 text-muted-foreground/30 mb-2" />
            ...
            <div className="flex gap-4">
              {safeData.map((item) => (
                <div
                  ...
                  className="inline-block w-44 shrink-0 cursor-pointer group bg-muted/20 border border-border/40 hover:border-primary/20 rounded-xl overflow-hidden focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                >
                  ...
                  <div className="p-2.5">
                    <h4 className="font-extrabold text-[11px] tracking-tight text-white leading-tight truncate group-hover:text-primary">
                      {item.title || item.name}
                    </h4>
