    ...
    fetchApi('/tv/genres')
      .then((data) => {
        if (data && typeof data === 'object') {
          ...
          const resolved = Array.isArray(obj.genres) ? obj.genres : []
  ...
  const loadShows = useCallback(async () => {
    ...
    try {
      if (selectedGenres.length > 1) {
        // Multi-genre filtering fallback: fetch all shows for each selected genre in parallel,
        ...
        const fetches = selectedGenres.map(async (genreName) => {
          ...
          const res = await fetchApi(`/tv?${params}`)
          return resolveList(res)
        })

        const lists = await Promise.all(fetches)

        // Intersect the lists by show ID (optimized: preserve order of first list, O(1) lookups)
        ...
        const data = await fetchApi(`/tv?${params}`)
        setShows(resolveList(data))
        setPagination(resolvePagination(data))
      }
    } catch {
      setShowsError('Could not load TV shows. Please try again.')
      setShows([])
      setPagination(null)
    } finally {
          ...
          <div className="w-full h-85 rounded-2xl relative overflow-hidden group border border-border/20 shadow-2xl">
            ...
            <div className="absolute inset-y-0 left-0 w-full md:w-3/5 p-8 flex flex-col justify-center gap-4">
              ...
              <p className="text-xs text-muted-foreground/80 font-bold leading-relaxed line-clamp-3 max-w-md">
                {spotlight.overview || 'Discover this featured TV show in our collection.'}
              </p>
              ...
              <div className="flex gap-2.5 flex-wrap">
                <Button
                  onClick={() => navigate(`/tv/${getSlug(spotlight)}`)}
