...
const resolveItem = (res: unknown): MediaItem | null => {
  if (res && typeof res === 'object') {
    ...
    // Others wrap it in a data, item, tvShow, media, or movie property
...
export default function TvShowSlugPage(): React.JSX.Element {
  ...
  const inWatchlist = show
    ? watchlist.some((w) => w.id === show.id && w.contentType === show.contentType)
    : false

  const toggleWatchlist = useCallback(() => {
    if (!show) return
    setWatchlist((prev) => {
      const exists = prev.some((w) => w.id === show.id && w.contentType === show.contentType)
      const updated = exists
        ? prev.filter((w) => !(w.id === show.id && w.contentType === show.contentType))
        : [...prev, show]
      ...
      <div className="relative w-full h-[65vh] min-h-112.5 overflow-hidden">
        ...
        <div className="absolute bottom-10 left-6 md:left-12 lg:left-24 z-10 max-w-4xl">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-6">
            ...
            {year && show.numberOfSeasons && <span className="text-white/40">•</span>}
            ...
            <div className="bg-white/2 border border-white/5 rounded-3xl p-6 flex flex-col gap-8 shadow-2xl backdrop-blur-sm">
              <div>
                ...
                <div className="text-4xl font-black flex items-center gap-3">
                  {show.voteAverage?.toFixed(1) || '0.0'}
